...
const CashSettlement = () => {
  ...
  const fetchCashRecord = async () => {
    try {
      ...
      console.error("Error fetching cash record:", err); // Log the error for debugging
  ...
  const handleGiveCash = async () => {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        const errorMsg = err.response?.data?.message || "Something went wrong while submitting.";
